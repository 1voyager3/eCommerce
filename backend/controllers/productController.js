...
const getProducts = asyncHandler( async ( req, res ) => {
  ...
  // @desc counting the products in db
  const count = await Product.countDocuments( { ...keyword } );

  // desc search by keyword (whole word or part of the word)
  // or retrieve all products
...
const createProductReview = asyncHandler( async ( req, res ) => {
  ...
  // @desc destruction from review object
  const {
    rating, comment
  } = req.body;
...
const getTopProducts = asyncHandler( async ( req, res ) => {
  ...
  // @desc -1 means to be sorted by negative one
